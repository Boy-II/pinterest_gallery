...
async def test_boards_handler_reports_config_error_without_crashing(monkeypatch):
    monkeypatch.setattr(pinterest_server, "_official_client", None)
    monkeypatch.setattr(
        pinterest_server, "_official_client_error", "PINTEREST_API_ENV must be sandbox or production"
    )
    client = await _make_client()
    async with client:
        resp = await client.get("/pinterest_gallery/boards")
        ...
        payload = await resp.json()
        assert "PINTEREST_API_ENV" in payload["error"]


async def test_board_pins_handler_reports_config_error_without_crashing(monkeypatch):
    monkeypatch.setattr(pinterest_server, "_official_client", None)
    monkeypatch.setattr(
        pinterest_server, "_official_client_error", "PINTEREST_API_ENV must be sandbox or production"
    )
    client = await _make_client()
    async with client:
        resp = await client.get("/pinterest_gallery/boards/board_1/pins")
        assert resp.status == 500
        payload = await resp.json()
        assert "PINTEREST_API_ENV" in payload["error"]
